...
LABEL org.opencontainers.image.description="Search and query government open-data portals (Socrata SODA API) via MCP. STDIO or Streamable HTTP."
...
RUN mkdir -p /var/log/socrata-mcp-server && chown -R bun:bun /var/log/socrata-mcp-server
...
ENV LOGS_DIR="/var/log/socrata-mcp-server"
