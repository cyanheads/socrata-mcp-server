...
- **Ask for missing input by returning, not awaiting.** Read `ctx.inputs`, then `return ctx.requestInput(...)` for anything still missing.
...
| `npm run release:github` | Create GitHub Release from the current annotated tag — attach `.mcpb` if present |
